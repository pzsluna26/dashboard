...
export default function NetworkGraph({
  ...
  // 우측 카드 탭(토글): 찬성·개정강화 / 찬성·폐지약화 / 반대
  const [activeTab, setActiveTab] = useState<'agree' | 'repeal' | 'disagree'>('agree');
  useEffect(() => { setActiveTab('agree'); }, [selected]);
  ...
  useEffect(() => {
    ...
    const idx = selected && selected.type === "incident"
        ...
        {!selected ? (
          ...
          <div className="mt-4 text-sm text-neutral-700 space-y-4">
            ...
            {(() => {
              ...
              const TabBtn = ({ keyName, label, count }:{ keyName: 'agree'|'repeal'|'disagree'; label: string; count: number }) => (
                <button
                  onClick={() => setActiveTab(keyName)}
                  className={`text-xs px-2 py-1 rounded-lg border transition-colors whitespace-nowrap ${activeTab === keyName ? 'bg-neutral-800 text-white border-neutral-800' : 'bg-white/70 text-neutral-700 border-neutral-300 hover:bg-white'}`}
                >
              ...
              return (
                <div className="space-y-3">
                  ...
                  <ul className="list-disc pl-4 text-xs text-neutral-700 space-y-1">
                    {(current.length > 0 ? current : ['(표본 없음)']).slice(0, 5).map((t, i) => (
                      <li key={i} className="line-clamp-1">{truncateText(t ?? '', 20)}</li>
                    ))}
                  </ul>
                </div>
              );
